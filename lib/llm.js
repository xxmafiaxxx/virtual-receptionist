...
async function callLocal({ prompt, system, maxTokens, timeoutMs }) {
  ...
  const payload = {
    ...
    stream: false,
    keep_alive: '30m',
    // thinking models burn budget inside <think> before answering — never let the visible answer get squeezed out
    options: { num_predict: Math.max(maxTokens, 200) },
  }
  let r = await fetch(`${base}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: AbortSignal.timeout(timeoutMs), body: JSON.stringify({ ...payload, think: false }) })
  if (!r.ok) {
    const errText = await r.text().catch(() => '')
    if (/think/i.test(errText)) {
      // model or server does not support the think flag — retry without it
      r = await fetch(`${base}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: AbortSignal.timeout(timeoutMs), body: JSON.stringify(payload) })
    } else throw new Error(errText.slice(0, 200) || `Ollama returned ${r.status}`)
  }
  const d = await r.json().catch(() => ({}))
